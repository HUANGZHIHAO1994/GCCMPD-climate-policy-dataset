...
def get_data():
    data = pd.read_excel("/home/zhhuang/climate_policy_paper/code/data/ALL_POLICIES_EN_FOR_TOPIC.xlsx")
    return data


def get_bm25_score(top_ratio):
    iea_cp_cclw_column = ['Year', 'db_source', 'Policy', 'ISO_code']
    column = ['Year', 'Source', 'Policy', 'ISO_code']
    ...
    iea_cp_cclw_df = iea_cp_cclw_df.rename(
        columns={"Source": 'URL', 'db_source': "Source"})

    df_docs = pd.read_excel("/home/zhhuang/climate_policy_paper/code/data/ALL_POLICIES_EN_FOR_TOPIC.xlsx")
    ...
    grouped = df_all.groupby(["Year", "ISO_code"])["Policy"]
    ...
    bm25_scr_list.sort()
    # ratio = int(2940 / 13298 * len(df_bm25["bm25_score_first"].to_list()))
...
def save(df_result, del_list, score):
    ...
    with pd.ExcelWriter(
            "/home/zhhuang/climate_policy_paper/code/data/bm25_{}_move_duplicate_result_for_topic.xlsx".format(
...
if __name__ == '__main__':
    ...
    BM25_experience_judgment = get_bm25_score(2940)
    ...
    grouped = df.groupby(["Year", "ISO_code"])["Policy"]
