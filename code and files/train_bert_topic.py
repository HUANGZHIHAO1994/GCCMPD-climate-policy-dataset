...
def analyze(args):
    ...
    topic_model.save(os.path.join(args.save_dir, f'bert_topic_model'))
    ...
    topic_df['Index'] = df["Index"].to_list()
    ...
    #     topic_df["ISO"] = [f"{sample['iso']}" for sample in policy_set]
    #     topic_df['Annex'] = [f"{sample['annex']}" for sample in policy_set]
    #     topic_df['IPCC_Region'] = [f"{sample['ipcc_region']}" for sample in policy_set]

    with pd.ExcelWriter("Topic.xlsx", engine='xlsxwriter',
...
def parser():
    ...
    parser.add_argument('--nr_topics', default=60, type=int)
